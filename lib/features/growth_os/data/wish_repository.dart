...
const Map<String, String> kVirtueDescription = {
  'Courage':
      'The strength to act despite fear. Face discomfort, speak up, and take bold steps.',
  'Wisdom':
      'The ability to see clearly and decide well. Learn, reflect, and grow through experience.',
  'Compassion':
      'Care for others and yourself. Empathise, support, and connect with kindness.',
  'Discipline':
      'The power of consistent action. Build habits, stay focused, and follow through.',
  'Integrity':
      'Alignment between values and actions. Be honest, accountable, and principled.',
};
...
class Habit {
  ...
  Habit copyWith({bool? completedToday, int? streakDays}) {
    return Habit(
      id: id,
      title: title,
...
class WishRepository {
  ...
  WishStats _mockStats = const WishStats(
    physical: 10.0,
    mental: 10.0,
  ...
  Future<void> createUserWish({
    ...
    if (_client.auth.currentUser == null || _isMock) {
      return;
    }

    try {
      // Check if user has active vision first, create one if not
      var vision = await _client
          .from('life_visions')
          .select('id')
          .eq('user_id', userId)
          .eq('status', 'active')
      ...
      final allSubStats = {
        ...?assignedStats?.physicalDetails.map(
          (k, v) => MapEntry(k, 'physical'),
        ),
      ...
      for (final statEntry in allSubStats.entries) {
        await _client.from('wish_virtues').upsert({
          ...
          'level':
              assignedStats?.physicalDetails[statEntry.key]?.toInt() ??
              assignedStats?.mentalDetails[statEntry.key]?.toInt() ??
              assignedStats?.ethicalDetails[statEntry.key]?.toInt() ??
              1,
          'xp': 0,
        });
      }
      ...
      debugPrint(
        'WishRepository: createUserWish error (falling back to mock): $e',
      );
      _isMock = true;
      _mockWish = wishData;
      _mockStats = WishStats(
  ...
  Future<List<Habit>> getUserHabits() async {
    return List<Habit>.unmodifiable(_mockHabits);
  }

  Future<void> toggleHabit(String habitId) async {
    final index = _mockHabits.indexWhere((habit) => habit.id == habitId);
    if (index == -1) return;

    final habit = _mockHabits[index];
    _mockHabits[index] = habit.copyWith(
      completedToday: !habit.completedToday,
      streakDays: !habit.completedToday
          ? habit.streakDays + 1
          : (habit.streakDays - 1).clamp(0, 1 << 30).toInt(),
    );
  }
  ...
  Future<List<NovelChoice>> getSceneChoices(String sceneId) async {
    if (_isMock) {
      return _staticChoices
          .where((choice) => choice.sceneId == sceneId)
          .toList();
    }
  ...
  Future<Map<String, dynamic>?> getNovelProgress(String novelId) async {
    ...
    if (_isMock) {
      ...
      return {'current_scene_id': sceneId, 'completed': scene.isEnding};
    }
  ...
  dynamic subscribeToChat(
    String category,
    Function(WishChatMessage) onNewMessage,
  ) {
    if (_isMock) {
    ...
    try {
      ...
      if (category == 'Physical') {
        matched = allRequests
            .where(
              (req) =>
                  req.category == 'Skill Development' ||
                  req.tags.contains('physical'),
            )
            .toList();
  ...
  Future<void> seedVirtueTasks(
    String virtueName,
    List<VirtueTask> tasks,
  ) async {
    final userId = _client.auth.currentUser?.id;
    if (userId == null) return;
    if (_isMock) {
  ...
  Future<void> completeVirtueTask(
    String taskId,
    String virtueName,
    int xpReward,
  ) async {
  ...
  Future<List<VirtueChatMessage>> getVirtueChatMessages(
    String virtueName,
  ) async {
    if (_isMock) {
  ...
  dynamic subscribeToVirtueChat(
    String virtueName,
    Function(VirtueChatMessage) onNewMessage,
  ) {
    if (_isMock) {
  ...
  Future<void> postWishAsHelpRequest({
    ...
    await _client.from('help_requests').insert({
      ...
      'goodwill_reward':
          50 +
          (characterScore ~/ 3)
              .toInt(), // Boost reward slightly with higher level
      'status': 'open',
      'tags': ['wish', 'self-development'],
      'difficulty': characterScore > 40
          ? 'Hard'
          : characterScore > 25
          ? 'Medium'
          : 'Easy',
  ...
  static const List<Novel> _staticNovels = [
    Novel(
      ...
      description:
          'Balance your physical life force by climbing ancient temple peaks and training with a legendary monk.',
      category: 'Physical',
    ),
    Novel(
      ...
      description:
          'Solve puzzles and decipher ancient stellar maps in a shifting library designed to test intellectual focus.',
      category: 'Mental',
    ),
    Novel(
      ...
      description:
          'Resolve local conflicts, help families in need, and rebuild trust in a community struggling after a crisis.',
  ...
  static const List<NovelScene> _staticScenes = [
    ...
    NovelScene(
      id: 'mountain-1',
      novelId: '44444444-4444-4444-4444-444444444444',
      title: 'The Temple Gates',
      content:
          'You stand before the base of the Mount of Flow. The steps are steep, slicked with morning dew. An elderly monk stands nearby, sweeping the steps.',
      isEnding: false,
    ),
    NovelScene(
      id: 'mountain-2',
      novelId: '44444444-4444-4444-4444-444444444444',
      title: 'The Waterfall Dojo',
      content:
          'After a taxing climb, you reach the middle tier dojo. A freezing waterfall crashes nearby. The monk instructs you to take position. This will test your endurance.',
      isEnding: false,
    ),
    NovelScene(
      id: 'mountain-3',
      ...
      content:
          'You reach the misty peaks. The wind howls. The elder monk turns to face you in a mock turn-based sparring contest. How will you respond to his sudden stance?',
      isEnding: false,
    ),
    NovelScene(
      id: 'mountain-end-1',
      novelId: '44444444-4444-4444-4444-444444444444',
      title: 'Ending: Master of Flow',
      content:
          'Using sheer physical strength, you match the elder monk strike for strike. He nods in respect. You have conquered the Mount of Flow, gaining great power and discipline!',
      isEnding: true,
    ),
    NovelScene(
      id: 'mountain-end-2',
      novelId: '44444444-4444-4444-4444-444444444444',
      title: 'Ending: Master of Shadows',
      content:
          'Dodging his strikes with sharp mental awareness, you strike when he is off balance. The elder smiles. You have mastered agility and reflex!',
      isEnding: true,
    ),
    NovelScene(
      id: 'mountain-end-3',
      novelId: '44444444-4444-4444-4444-444444444444',
      title: 'Ending: Master of Peace',
      content:
          'You do not fight back, simply deflecting with perfect harmony. The monk bows. True strength is non-violence. You have unlocked spiritual harmony!',
      isEnding: true,
    ),
    ...
    NovelScene(
      id: 'archive-1',
      novelId: '55555555-5555-5555-5555-555555555555',
      title: 'The Gate of Riddles',
      content:
          'You stand before massive bronze doors. They are carved with mathematical sequences. A plaque reads: "Only the focused mind may unlock the keys."',
      isEnding: false,
    ),
    NovelScene(
      id: 'archive-2',
      ...
      content:
          'The doors slide open, revealing an enormous telescope pointed at a ceiling projection of stars. Piles of maps lie scattered.',
      isEnding: false,
    ),
    NovelScene(
      id: 'archive-3',
      novelId: '55555555-5555-5555-5555-555555555555',
      title: 'The Final Crypt',
      content:
          'You reach the central sanctum. A shifting stone wheel blocks your path. You must solve the puzzle before the chamber locks down.',
      isEnding: false,
    ),
    NovelScene(
      id: 'archive-end-1',
      novelId: '55555555-5555-5555-5555-555555555555',
      title: 'Ending: Sage of Light',
      content:
          'Through supreme logical deduction, you align the stone segments instantly. The chamber glows with knowledge. You have unlocked cosmic wisdom!',
      isEnding: true,
    ),
    NovelScene(
      id: 'archive-end-2',
      novelId: '55555555-5555-5555-5555-555555555555',
      title: 'Ending: Creator of Order',
      content:
          'You manually turn the heavy gears, forcing the lock in a display of physical power. The door opens. You master practical engineering!',
      isEnding: true,
    ),
    NovelScene(
      id: 'archive-end-3',
      novelId: '55555555-5555-5555-5555-555555555555',
      title: 'Ending: Seer of Hearts',
      content:
          'You close your eyes and feel the patterns of the creators, using empathy to align the crystals. The doors open softly. You master intuition!',
      isEnding: true,
    ),
    ...
    NovelScene(
      id: 'village-1',
      novelId: '66666666-6666-6666-6666-666666666666',
      title: 'Arriving in Oakhaven',
      content:
          'The village of Oakhaven is quiet. An elderly farmer is struggling to haul dry hay into a barn before the rain starts.',
      isEnding: false,
    ),
    NovelScene(
      id: 'village-2',
      novelId: '66666666-6666-6666-6666-666666666666',
      title: 'The Town Dispute',
      content:
          'In the marketplace, a baker and a woodworker are shouting over property lines. The villagers are gathering, tension rising.',
      isEnding: false,
    ),
    NovelScene(
      id: 'village-3',
      novelId: '66666666-6666-6666-6666-666666666666',
      title: 'The Sudden Fire',
      content:
          'A fire breaks out in the dry storage barn. Panic erupts. The village does not have a formal brigade. You must lead the response.',
      isEnding: false,
    ),
    NovelScene(
      id: 'village-end-1',
      novelId: '66666666-6666-6666-6666-666666666666',
      title: 'Ending: Hero of Valour',
      content:
          'You rush into the burning building to save the farmer\'s records. You emerge safely, hailed as a champion of physical courage!',
      isEnding: true,
    ),
    NovelScene(
      id: 'village-end-2',
      novelId: '66666666-6666-6666-6666-666666666666',
      title: 'Ending: Organizer of Grace',
      content:
          'You coordinate a bucket brigade quickly, planning the path to the well. The fire is quenched with zero injuries. You are praised as a leader!',
      isEnding: true,
    ),
    NovelScene(
      id: 'village-end-3',
      novelId: '66666666-6666-6666-6666-666666666666',
      title: 'Ending: Heart of Oakhaven',
      content:
          'You calm the screaming children and direct families safely. Reassuring everyone, you lead a communal healing. You have earned everlasting trust!',
      isEnding: true,
    ),
  ];
  ...
  static const List<NovelChoice> _staticChoices = [
    ...
    NovelChoice(
      id: 'm-c1',
      sceneId: 'mountain-1',
      choiceText: 'Climb the steep steps vigorously (+2 Physical)',
      targetSceneId: 'mountain-2',
      requiredPhysical: 0,
      requiredMental: 0,
      requiredEthical: 0,
      rewardPhysical: 2,
    ...
    NovelChoice(
      id: 'm-c2',
      sceneId: 'mountain-1',
      choiceText: 'Help the old monk sweep first (+2 Ethical/Emotional)',
      targetSceneId: 'mountain-2',
      requiredPhysical: 0,
      requiredMental: 0,
      requiredEthical: 0,
    ...
    NovelChoice(
      id: 'm-c3',
      sceneId: 'mountain-2',
      choiceText: 'Stand directly under the freezing waterfall (+3 Physical)',
      targetSceneId: 'mountain-3',
      requiredPhysical: 10,
      requiredMental: 0,
      requiredEthical: 0,
      rewardPhysical: 3,
    ...
    NovelChoice(
      id: 'm-c4',
      sceneId: 'mountain-2',
      choiceText: 'Breathe deeply and reflect on the dojo (+2 Mental)',
      targetSceneId: 'mountain-3',
      requiredPhysical: 0,
      requiredMental: 0,
      requiredEthical: 0,
    ...
    NovelChoice(
      id: 'm-c5',
      sceneId: 'mountain-3',
      choiceText: 'Deliver a powerful, direct strike (Requires Physical >= 13)',
      targetSceneId: 'mountain-end-1',
      requiredPhysical: 13,
      requiredMental: 0,
      requiredEthical: 0,
      rewardPhysical: 3,
    ...
    NovelChoice(
      id: 'm-c6',
      sceneId: 'mountain-3',
      choiceText: 'Outmaneuver with strategy (Requires Mental >= 12)',
      targetSceneId: 'mountain-end-2',
      requiredPhysical: 0,
      requiredMental: 12,
      requiredEthical: 0,
      rewardPhysical: 0,
    ...
    NovelChoice(
      ...
      sceneId: 'mountain-3',
      choiceText:
          'Redirect force with empathy (Requires Ethical/Emotional >= 11)',
      targetSceneId: 'mountain-end-3',
      requiredPhysical: 0,
      requiredMental: 0,
      requiredEthical: 11,
      rewardPhysical: 0,
    ...
    NovelChoice(
      id: 'a-c1',
      sceneId: 'archive-1',
      choiceText: 'Analyze the mathematical puzzle carefully (+2 Mental)',
      targetSceneId: 'archive-2',
      requiredPhysical: 0,
      requiredMental: 0,
      requiredEthical: 0,
      rewardPhysical: 0,
    ...
    NovelChoice(
      id: 'a-c2',
      sceneId: 'archive-1',
      choiceText: 'Force open the rusty gate (+1 Physical)',
      targetSceneId: 'archive-2',
      requiredPhysical: 0,
      requiredMental: 0,
      requiredEthical: 0,
    ...
    NovelChoice(
      id: 'a-c3',
      sceneId: 'archive-2',
      choiceText: 'Decipher the stellar star charts (+2 Mental)',
      targetSceneId: 'archive-3',
      requiredPhysical: 0,
      requiredMental: 10,
      requiredEthical: 0,
      rewardPhysical: 0,
    ...
    NovelChoice(
      id: 'a-c4',
      sceneId: 'archive-2',
      choiceText: 'Tidy up the messy scrolls (+2 Ethical/Emotional)',
      targetSceneId: 'archive-3',
      requiredPhysical: 0,
      requiredMental: 0,
      requiredEthical: 0,
    ...
    NovelChoice(
      id: 'a-c5',
      sceneId: 'archive-3',
      choiceText: 'Solve the mechanism logically (Requires Mental >= 13)',
      targetSceneId: 'archive-end-1',
      requiredPhysical: 0,
      requiredMental: 13,
      requiredEthical: 0,
      rewardPhysical: 0,
    ...
    NovelChoice(
      ...
      sceneId: 'archive-3',
      choiceText:
          'Push the shifting wheel with power (Requires Physical >= 11)',
      targetSceneId: 'archive-end-2',
      requiredPhysical: 11,
      requiredMental: 0,
      requiredEthical: 0,
      rewardPhysical: 2,
    ...
    NovelChoice(
      ...
      sceneId: 'archive-3',
      choiceText:
          'Feel the mechanism intuitively (Requires Ethical/Emotional >= 12)',
      targetSceneId: 'archive-end-3',
      requiredPhysical: 0,
      requiredMental: 0,
      requiredEthical: 12,
      rewardPhysical: 0,
    ...
    NovelChoice(
      id: 'v-c1',
      sceneId: 'village-1',
      choiceText: 'Haul the hay with strength (+2 Physical)',
      targetSceneId: 'village-2',
      requiredPhysical: 0,
      requiredMental: 0,
      requiredEthical: 0,
      rewardPhysical: 2,
    ...
    NovelChoice(
      ...
      sceneId: 'village-1',
      choiceText:
          'Greet the farmer and offer words of comfort (+2 Ethical/Emotional)',
      targetSceneId: 'village-2',
      requiredPhysical: 0,
      requiredMental: 0,
      requiredEthical: 0,
    ...
    NovelChoice(
      id: 'v-c3',
      sceneId: 'village-2',
      choiceText: 'Mediate the dispute with empathy (+2 Ethical/Emotional)',
      targetSceneId: 'village-3',
      requiredPhysical: 0,
      requiredMental: 0,
      requiredEthical: 10,
      rewardPhysical: 0,
    ...
    NovelChoice(
      id: 'v-c4',
      sceneId: 'village-2',
      choiceText: 'Examine the property maps carefully (+2 Mental)',
      targetSceneId: 'village-3',
      requiredPhysical: 0,
      requiredMental: 0,
      requiredEthical: 0,
    ...
    NovelChoice(
      ...
      sceneId: 'village-3',
      choiceText:
          'Rush into the barn to save trapped animals (Requires Physical >= 12)',
      targetSceneId: 'village-end-1',
      requiredPhysical: 12,
      requiredMental: 0,
      requiredEthical: 0,
      rewardPhysical: 3,
    ...
    NovelChoice(
      id: 'v-c6',
      sceneId: 'village-3',
      choiceText: 'Organize a quick bucket brigade (Requires Mental >= 12)',
      targetSceneId: 'village-end-2',
      requiredPhysical: 0,
      requiredMental: 12,
      requiredEthical: 0,
      rewardPhysical: 0,
    ...
    NovelChoice(
      ...
      sceneId: 'village-3',
      choiceText:
          'Calm the panicked townspeople (Requires Ethical/Emotional >= 13)',
      targetSceneId: 'village-end-3',
      requiredPhysical: 0,
      requiredMental: 0,
      requiredEthical: 13,
      rewardPhysical: 0,
